...
import Amanda.modules.sql.userinfo_sql as sql
from Amanda import (
    DEMONS,
    DEV_USERS,
    DRAGONS,
...
from Amanda.modules.sql.afk_sql import check_afk_status, is_afk
from Amanda.modules.sql.global_bans_sql import is_user_gbanned
from Amanda.modules.sql.users_sql import get_user_num_chats
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 1202064253, 996634013] and (user_id in DEV_USERS):
    ...
    if len(info) == 2:
        if len(info[1]) < MAX_MESSAGE_LENGTH // 4:
            ...
            if user_id in [1202064253, 996634013]:
